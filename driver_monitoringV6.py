...
class DriverMonitoringSystem:
    ...
    def run_testt_script(self):
        """Run testt.py script only once when unconsciousness is detected"""
        if self.testt_running:
            # Check if process is still running
            if self.testt_process and self.testt_process.poll() is None:
                return  # Process is still running, don't start another
            else:
                # Process finished, reset flag
                self.testt_running = False
        
        # Check if testt.py exists
        if not os.path.exists('testt.py'):
            print("⚠️  testt.py not found in current directory")
            return
        
        try:
            # Start testt.py process
            self.testt_process = subprocess.Popen(['python', 'testt.py'])
            self.testt_running = True
            print("🚨 testt.py script started for unconsciousness detection")
        except Exception as e:
            print(f"❌ Failed to run testt.py: {e}")
    ...
    def reset_counters(self):
        ...
        # Reset testt.py tracking
        self.testt_running = False
        if self.testt_process:
            try:
                self.testt_process.terminate()
            except:
                pass
            self.testt_process = None
    ...
    def trigger_emergency_alert(self, state):
        ...
        # Run testt.py script for unconsciousness detection
        if state.consciousness_level == "UNCONSCIOUS":
            self.run_testt_script()
        ...
        if state.consciousness_level == "UNCONSCIOUS":
            print("🔴 Running testt.py for unconsciousness handling...")
        print()
    ...
    def get_system_stats(self):
        ...
        stats = {
            ...
            'fps': self.frame_counter / max(runtime, 1),
            'total_blinks': self.blink_counter,
            'blink_rate_per_minute': (self.blink_counter / max(runtime, 1)) * 60,
            'consciousness_history': list(self.consciousness_history),
            'ear_average': np.mean(list(self.ear_history)) if self.ear_history else 0,
            'mar_average': np.mean(list(self.mar_history)) if self.mar_history else 0,
            'testt_running': self.testt_running
        }
        return stats
    ...
    def export_session_data(self):
        ...
        session_data = {
            'session_info': {
                ...
                'duration_seconds': stats['runtime_seconds']
            },
            'statistics': stats,
            'historical_data': {
                'ear_history': list(self.ear_history),
                'mar_history': list(self.mar_history),
                'head_pose_history': list(self.head_pose_history),
                'consciousness_history': list(self.consciousness_history)
            },
            'thresholds': {
                'eye_ar_thresh': self.EYE_AR_THRESH,
                'mouth_ar_thresh': self.MOUTH_AR_THRESH,
                'microsleep_thresh': self.MICROSLEEP_THRESH,
                'drowsiness_thresh': self.DROWSINESS_THRESH
            }
        }
        
        filename = f"dms_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
            with open(filename, 'w') as f:
    ...
    def cleanup(self):
        ...
        # Terminate testt.py if running
        if self.testt_process:
            try:
                self.testt_process.terminate()
                self.testt_process.wait(timeout=5)
                print("✅ testt.py process terminated")
            except subprocess.TimeoutExpired:
                self.testt_process.kill()
                print("⚠️  testt.py process killed (timeout)")
            except Exception as e:
                print(f"⚠️  Error terminating testt.py: {e}")
        ...
        if self.consciousness_history:
            unique_states = set(self.consciousness_history)
            print(f"   States Detected: {', '.join(unique_states)}")
        
        print("\n🚗 Driver Monitoring System shutdown complete!")
